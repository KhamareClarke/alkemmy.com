...
import { getRollOns, RollOn } from '@/lib/category-api';
...
export default function RollOnsPage() {
  const [products, setProducts] = useState<RollOn[]>([]);
  const [filteredProducts, setFilteredProducts] = useState<RollOn[]>([]);
  ...
  useEffect(() => {
    const loadRollOns = async () => {
      try {
        setLoading(true);
        const rollOns = await getRollOns();
        setProducts(rollOns);
        setFilteredProducts(rollOns);
      } catch (error) {
        console.error('Error loading roll-ons:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading roll-ons...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="text-center">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">⚡ Roll-ons</h1>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Targeted treatments on-the-go. Convenient roll-on formulas for quick and effective application.
            </p>
          </div>
        </div>
      </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-gray-500 text-lg">
                  {products.length === 0 ? 'No roll-ons found.' : 'No roll-ons match your filters.'}
                </p>
                {products.length > 0 && (
              ...
              <div className="mb-6">
                <p className="text-gray-600">
                  Showing {filteredProducts.length} of {products.length} roll-ons
                </p>
              </div>
            )}
