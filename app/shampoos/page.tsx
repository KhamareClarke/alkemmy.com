...
import { getShampoos, Shampoo } from '@/lib/category-api';
...
export default function ShampoosPage() {
  const [products, setProducts] = useState<Shampoo[]>([]);
  const [filteredProducts, setFilteredProducts] = useState<Shampoo[]>([]);
  ...
  useEffect(() => {
    const loadShampoos = async () => {
      try {
        setLoading(true);
        const shampoos = await getShampoos();
        setProducts(shampoos);
        setFilteredProducts(shampoos);
      } catch (error) {
        console.error('Error loading shampoos:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading shampoos...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="text-center">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">🚿 Shampoos & Conditioners</h1>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Gentle cleansing for healthy hair. Natural shampoos and conditioners formulated for all hair types.
            </p>
          </div>
        </div>
      </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-gray-500 text-lg">
                  {products.length === 0 ? 'No shampoos found.' : 'No shampoos match your filters.'}
                </p>
                {products.length > 0 && (
              ...
              <div className="mb-6">
                <p className="text-gray-600">
                  Showing {filteredProducts.length} of {products.length} shampoos
                </p>
              </div>
            )}
