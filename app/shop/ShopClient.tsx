...
function ShopClientContent({ initialProducts, initialCategories, initialTags }: ShopClientProps) {
  ...
  const [categories] = useState<string[]>(['all', ...initialCategories]);
  ...
  useEffect(() => {
    ...
    if (selectedFilters.length > 0) {
      filtered = filtered.filter(product => 
        product.tags && selectedFilters.some(tag => product.tags?.includes(tag))
      );
    }
  ...
  }, [selectedCategory, selectedFilters, debouncedSearchTerm, allProducts]);
  ...
  const productsByCategory = products.reduce((acc, product) => {
    if (!acc[product.category || 'Uncategorized']) {
      acc[product.category || 'Uncategorized'] = [];
    }
    acc[product.category || 'Uncategorized'].push(product);
    return acc;
