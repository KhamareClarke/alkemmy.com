...
export async function getAllProducts(filters?: ProductFilters) {
  ...
  // Fetch from all category tables in parallel
  const categoryPromises = categoryTables.map(async (table) => {
    try {
      let query = supabase
        .from(table)
        .select('*')
      ...
      if (data) {
        ...
        return data.map(product => ({
          ...product,
          category: normalizeCategoryName(product.category || getCategoryFromTable(table))
        }));
      }
      return [];
    } catch (err) {
  ...
  // Fetch main products table in parallel
  const mainProductsPromise = getProducts(filters)
  ...
  const results = await Promise.all([...categoryPromises, mainProductsPromise]);
